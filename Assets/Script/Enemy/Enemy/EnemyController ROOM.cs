...
public class EnemyControllerROOM : MonoBehaviour
{
    [SerializeField] float sightRange; // ระยะการมองเห็นของศัตรู
   ...
   [SerializeField] float maxY; // ขอบเขตบน
    ...
    void Update()
    {
        ...
        // ตรวจสอบการมองเห็น Player ก่อนยิงกระสุน
        /*if (IsPlayerInSight())
        {
            // ยิงจรวดตามเวลาที่กำหนด
            if (Time.time > nextFire)
        ...
        // ตรวจสอบการมองเห็น Player โดยใช้ Raycast 
        if (IsPlayerInSight())
        {
            ShootAtPlayer();
        }
    ...
    void Shoot()
    {
        ...
        {
            Debug.LogError("No Rigidbody2D found on bullet prefab.");
        }
    }
    ...
    bool IsPlayerInSight()
    {
       ...
       // RaycastHit2D hit = Physics2D.Raycast(transform.position, Vector2.down, sightRange);
       RaycastHit2D hit = Physics2D.Raycast(transform.position, Vector2.left, sightRange);
       
        if (hit.collider != null && hit.collider.CompareTag("Player"))
        {
            Debug.Log("Player detected in sight range.");
            return true;
        }
    ...
    void ShootAtPlayer()
    {
        // ตรวจสอบเวลาสำหรับยิงกระสุน
        if (Time.time > nextFire)
        {
            nextFire = Time.time + fireRate;

            // ยิงกระสุนไปในทิศทางของ Player
            GameObject bullet = Instantiate(bulletPrefab, transform.position, Quaternion.identity);
            Rigidbody2D rb = bullet.GetComponent<Rigidbody2D>();

            if (rb != null)
            {
                rb.velocity = Vector2.left * bulletSpeed; // ยิงกระสุนไปทางซ้าย
            }
            else
            {
                Debug.LogError("No Rigidbody2D found on bullet prefab.");
